...
def applicant_dashboard(request):
    ...
    recent_applications = Application.objects.filter(applicant=user).order_by('-submitted_date')[:1]
...
def get_recent_applications_json(request):
    user = request.user
    applications = Application.objects.filter(applicant=user).order_by('-submitted_date')[:1]
...
def application_form_view(request):
    if request.method == 'POST':
        ...
        app = Application(
            ...
            btech2_sem3=data.get('btech2_sem3'),
            btech2_sem4=data.get('btech2_sem4'),
            btech3_sem5=data.get('btech3_sem5'),
            btech3_sem6=data.get('btech3_sem6'),
            btech4_sem7=data.get('btech4_sem7'),
            btech4_sem8=data.get('btech4_sem8'),
            mtech1_sem1=data.get('mtech1_sem1'),
            mtech1_sem2=data.get('mtech1_sem2'),
            mtech2_sem3=data.get('mtech2_sem3'),
            mtech2_sem4=data.get('mtech2_sem4'),
            ...
            cv_upload=files.get('cvUpload'),
            identification_doc=files.get('identificationDoc'),
...
def application_status_view(request):
    user = request.user
    applications = Application.objects.filter(applicant=user).order_by('-submitted_date')
    ...
    for app in applications:
        ...
        context = {
            ...
            "applications_json": json.dumps(apps_data)  # You must use json.dumps()
        }
